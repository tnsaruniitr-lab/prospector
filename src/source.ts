...
export async function source(opts: SourceOpts) {
  ...
  // Insert prospects with a qualify-at-discovery gate: only website + enough
  // reviews go into the research queue (research_status='pending'); the rest are
  // 'skip'. on_conflict keeps prior rows/audits intact.
  const minRev = playbook.qualify.minReviews;
  let inserted = 0, queued = 0;
  for (const p of prospects) {
    const fewReviews = p.reviewCount != null && p.reviewCount < minRev;
    const qualStatus = !p.website ? "no_website" : fewReviews ? "low_priority" : "new";
    const qualReason = !p.website
      ? "No website — phone/walk-in motion, not web audit."
      : fewReviews ? `Only ${p.reviewCount} reviews (< ${minRev}) — likely inactive.` : null;
    const researchStatus = p.website && !fewReviews ? "pending" : "skip";
    if (researchStatus === "pending") queued++;
    const rows = await q<{ id: string }>(
      `insert into ${T.prospects}
        (campaign_id, playbook, place_id, name, website, domain, phone, address,
         city, google_maps_uri, rating, review_count, types, qual_status, qual_reason, research_status)
      ...
      [
        ...
        qualStatus, qualReason, researchStatus,
      ],
    );
    if (rows.length) inserted++;
  }
  console.log(`[source] ${queued} qualified into the research queue (website + ≥${minRev} reviews).`);
